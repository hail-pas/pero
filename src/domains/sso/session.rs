...
use crate::domains::sso::models::SsoSession;
use crate::shared::error::AppError;

const SSO_SESSION_PREFIX: &str = "sso_session:";
...
pub async fn create(pool: &Pool, session: &SsoSession) -> Result<String, AppError> {
    let id = uuid::Uuid::new_v4().to_string();
    let key = format!("{}{}", SSO_SESSION_PREFIX, id);
    cache::set_json(pool, &key, session, SSO_SESSION_TTL_SECONDS).await?;
    Ok(id)
}
...
pub async fn update(pool: &Pool, session_id: &str, session: &SsoSession) -> Result<(), AppError> {
    let key = format!("{}{}", SSO_SESSION_PREFIX, session_id);
    cache::set_json(pool, &key, session, SSO_SESSION_TTL_SECONDS).await
}
