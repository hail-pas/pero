...
use crate::domain::identity::repo::{IdentityStore, SessionStore, UserStore};
...
impl AuthService {
    ...
    async fn load_password_credential(identities: &dyn IdentityStore, user_id: Uuid) -> Result<String, AppError> {
        let identity =
            identities.find_by_user_and_provider(user_id, PROVIDER_PASSWORD)
                .await?
    ...
    pub async fn register_user_with_password(
        ...
        let password_hash = service::hash_password(password)?;
        users.check_new_user_conflicts(username, email, phone).await?;
    ...
    pub async fn change_password(
        ...
        identities.update_credential(user_id, PROVIDER_PASSWORD, &new_hash).await?;
