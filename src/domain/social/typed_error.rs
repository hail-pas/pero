...
impl From<SocialTypedError> for AppError {
    fn from(e: SocialTypedError) -> AppError {
        match e {
            ...
            SocialTypedError::ProviderDisabled => AppError::BadRequest("social provider is disabled".into()),
            SocialTypedError::LoginFailed(msg) => AppError::BadRequest(format!("social login failed: {msg}")),
            SocialTypedError::StateInvalid => AppError::BadRequest("invalid or expired social login state".into()),
        }
    }
}
