...
use crate::domain::federation::error::social_login_failed;
...
pub async fn fetch_userinfo(
    ...
    let raw = http
        .get_bearer(&provider.userinfo_url, access_token)
        .await
    ...
    if provider.name == "github" && info.email.is_none() {
        if let Ok(github_resp) = http
            .get_bearer("https://api.github.com/user/emails", access_token)
            .await
        {
