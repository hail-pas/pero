...
#[tokio::test]
async fn evaluate_with_domain_resource_type() {
    let mut ta = build_app().await;
    let fx = ta.register_default_user().await;
    ta.grant_api_access(fx.user_id).await;
    let app_fx = ta.create_test_app(&fx.access_token).await;

    let policy_name = unique_name("domain-res");
    let (status, body) = send_request(
        &mut ta.app,
        hyper::Method::POST,
        "/api/policies",
        Some(serde_json::json!({
            ...
            "conditions": [
                ...
                {"condition_type": "action", "key": "type", "operator": "eq", "value": "read"},
            ],
        })),
        Some(&fx.access_token),
    )
...
#[tokio::test]
async fn evaluate_domain_action_type_deny() {
    let mut ta = build_app().await;
    let fx = ta.register_default_user().await;
    ta.grant_api_access(fx.user_id).await;
    let app_fx = ta.create_test_app(&fx.access_token).await;

    let policy_name = unique_name("domain-act-deny");
    let (status, body) = send_request(
        &mut ta.app,
        hyper::Method::POST,
        "/api/policies",
        Some(serde_json::json!({
            ...
            "conditions": [
                {"condition_type": "resource", "key": "type", "operator": "eq", "value": "user"},
                {"condition_type": "action", "key": "type", "operator": "eq", "value": "delete"},
            ],
        })),
        Some(&fx.access_token),
    )
...
#[tokio::test]
async fn policy_cache_invalidated_on_update() {
    let mut ta = build_app().await;
    let fx = ta.register_default_user().await;
    ta.grant_api_access(fx.user_id).await;
    let app_fx = ta.create_test_app(&fx.access_token).await;
