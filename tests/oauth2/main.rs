...
#[tokio::test]
async fn revoke_returns_ok_for_token_owned_by_another_client() {
    let mut ta = build_app().await;
    let fx = ta.register_default_user().await;
    ta.grant_api_access(fx.user_id).await;

    let (app_fx, client_fx_a) = create_app_with_client(&mut ta, &fx.access_token).await;
    let client_fx_b = ta
    ...
    let (status, body) = send_form_request(
        ...
        &[
            ...
            ("client_id", client_fx_b.client_id_str.clone()),
            ("client_secret", client_fx_b.client_secret.clone()),
        ],
    )
    .await;
