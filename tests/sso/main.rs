...
async fn count_cache_keys(cache: &pero::infra::cache::Pool, pattern: &str) -> usize {
...
#[tokio::test]
async fn sso_authorize_redirects_invalid_scope_to_client_callback() {
    let mut ta = build_app().await;
    let admin_fx = ta.register_default_user().await;
    ta.grant_api_access(admin_fx.user_id).await;
    let app_fx = ta.create_test_app(&admin_fx.access_token).await;
    ...
    let request = hyper::Request::builder()
        .method(hyper::Method::GET)
        .uri(format!(
            "/oauth2/authorize?client_id={}&redirect_uri=http://localhost:3000/callback&response_type=code&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256&scope=openid+admin&state=test-state",
            client_fx.client_id_str,
        ))
        .body(axum::body::Body::empty())
        .unwrap();
    ...
    assert!(
        ...
        "expected invalid_scope redirect: {location}"
    );
    assert!(
        location.contains("state=test-state"),
        "expected state to round-trip in redirect: {location}"
    );
...
#[tokio::test]
async fn sso_authorize_sets_cookie_from_config() {
    let mut ta = build_app().await;
    let admin_fx = ta.register_default_user().await;
    ta.grant_api_access(admin_fx.user_id).await;
    let app_fx = ta.create_test_app(&admin_fx.access_token).await;
    ...
    assert!(
        ...
        "expected SameSite from config: {set_cookie}"
    );
    assert!(
        set_cookie.contains("Max-Age=321"),
        "expected Max-Age from config: {set_cookie}"
    );
    assert!(
        !set_cookie.contains("Secure"),
        "cookie_secure=false should omit Secure: {set_cookie}"
    );
...
#[tokio::test]
async fn sso_register_rejects_invalid_phone() {
    let mut ta = build_app().await;
    let session_cookie = prepare_sso_session_cookie(&mut ta).await;
    ...
    let request = hyper::Request::builder()
        ...
        .body(axum::body::Body::from(
            "username=tester2&email=tester2%40example.com&password=password123&phone=invalid_phone!",
        ))
        .unwrap();
    let response = ta.app.clone().oneshot(request).await.unwrap();
    assert_eq!(response.status(), StatusCode::UNPROCESSABLE_ENTITY);
